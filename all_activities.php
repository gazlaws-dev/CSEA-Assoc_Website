...
<!--								<div id="courses" class="holder selected">
                        ...
                        <a href="all_activites.php"><button class="more_button">All Activities</button></a>
										</div>
								</div>
...
function closecredits() {
  ...
  return false;
    ...
    actdb.orderByKey().on("child_added",function(snapshot){
      var actval=snapshot.val();
      //console.log(actval);
      var obj;
      var year=snapshot.key;
      if(forselected==0){
        $('#navigation').prepend('<li><a data-val="ref_'+year+'" onclick="changeTab(this)" class="selected_link">'+year+'</a></li>')
        $('.main_middle_container_full').prepend('<div id="ref_'+year+'" class="holder selected"><div class="middle_header">Activities in '+year+'</div><div class="content_holder"><ul id="ul_'+year+'" class="dynamic_jq"></ul></div></div>')
        forselected=1;
      }
      else{
        $('#navigation').prepend('<li><a data-val="ref_'+year+'" onclick="changeTab(this)" >'+year+'</a></li>')
        $('.main_middle_container_full').prepend('<div id="ref_'+year+'" class="holder"><div class="middle_header">Activities in '+year+'</div><div class="content_holder"><ul id="ul_'+year+'" class="dynamic_jq"></ul></div></div>')
      }
      ...
      //console.log(obj);
          for(var iter in obj){
            (function(cntr){
              ...
              var targeted="#ul_"+year;
              $(targeted).prepend('<a href="#0" class="'+targetact+'"><li ><div class="act_info '+imagetargetact+'"><div class="project-info"><h2>'+title+'</h2><p>'+short+'</p></div></div></li></a>');
/*              if(category=="talk"){
...
*/
      ...
      $('.main_middle_container_full').on('click', 'a', function(event) {
